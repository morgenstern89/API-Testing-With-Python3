...
class OrdersHelper(object):
    ...
    def __init__(self):
        self.cur_file_dir=os.path.dirname(os.path.realpath(__file__)) #This is how to get current path from python.
    ...
    def verify_order_is_created(order_json, exp_cust_id, exp_products):
        ...
        for product in exp_products:
            assert product[
                       'product_id'] in api_products_ids, f"created order does not have at least 1 expected product in DB." \
